...
def test(
    ...
    capacity_hospital - capacity of the hospital, which is the max number of people that could be waiting in all the queues together.
    ...
    p_arr_prob - a list of the relative probabilities for different patients arriving.
        The index in the list corresponds to patient's type. [1, 1, 4] means that patient of type 0 has a probability of 1/6 to arrive, while patient of type 2 has a probability of 4/6 to arrive at each step.
    ...
    feature - the currently used featurisation of the state space.
        The definition of each featurisation is in learning.py
